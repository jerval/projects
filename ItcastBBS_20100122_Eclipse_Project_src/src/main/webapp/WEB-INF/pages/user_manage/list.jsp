...
<table class="list">
    ...
    <tr class="item">
        ...
        <td><c:forEach items="${item.roles}" var="role" varStatus="status">
        		<span style="width: 80px; float: left;">${role.name }</span> 
        	</c:forEach>
        	<html:link action="/manage/user?method=selectRolesUI&id=${item.id}&pageNum=${param.pageNum}&pageSize=${param.pageSize}" 
        		style="float: right;">修改</html:link>
    	</td>
        <td class="${item.locked ? 'red' : 'normal' }">${item.locked }</td>
        <td><html:link action="/manage/user?method=lock&id=${item.id}&pageNum=${param.pageNum}&pageSize=${param.pageSize}" 
        		onclick="return confirmDel('确定要锁定此用户吗？')">锁定</html:link>
            <html:link action="/manage/user?method=unlock&id=${item.id}&pageNum=${param.pageNum}&pageSize=${param.pageSize}" 

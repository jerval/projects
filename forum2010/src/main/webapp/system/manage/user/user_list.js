$(function() {
	...
	$("input[name='update']").click(function() {
		...
		if (ids.length == 1) {
			location.assign("user_edit?id=" + ids[0] + "#update");
			return;
		}
	});
	$("input[name='detail']").click(function() {
		...
		if (ids.length == 1) {
			location.assign("user_detail?id=" + ids[0]);
			return;
		}
	});
				...
				success : function(data) {
					...
					if (pagination["list"]) {
						$.each(pagination["list"], function(i, n) {
							addData(n.id, n.username, n.type, n.dateCreated);
						});
					} else {
						$("#dataArea")
								.append(
										"<tr height='40px'><td align='center' colspan='5'>暂无数据</td></tr>");
					}
					pageBar("pageBar1", pagination["pageNo"],
							pagination["maxPages"], pagination["maxElements"]);
					pageBar("pageBar2", pagination["pageNo"],
							pagination["maxPages"], pagination["maxElements"]);
...
function addData(id, username, type, dateCreated) {
	$("#dataArea")
			.append(
